...
class ec_manufacturerwidget extends WP_Widget{
	...
	function form($instance){
		if( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'Filter by Manufacturer', 'wp-easycart' );
		}
	...
	function widget($args, $instance){
		...
		for( $i=0; $i<count( $manufacturers ); $i++ ){
			$manufacturers[$i]->name = wp_easycart_language( )->convert_text( $manufacturers[$i]->name );
		}
		
		if( file_exists( EC_PLUGIN_DATA_DIRECTORY . '/design/layout/' . get_option( 'ec_option_base_layout' ) . '/ec_manufacturer_widget.php' ) )	
			include( EC_PLUGIN_DATA_DIRECTORY . "/design/layout/" . get_option( 'ec_option_base_layout' ) . "/ec_manufacturer_widget.php");
		else
			include( EC_PLUGIN_DIRECTORY . "/design/layout/" . get_option( 'ec_option_latest_layout' ) . "/ec_manufacturer_widget.php");
